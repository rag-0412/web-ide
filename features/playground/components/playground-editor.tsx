...
export const PlaygroundEditor = ({
  ...
  const analyzeCodeContext = useCallback(
    (editor: any, position: any) => {
      ...
      const context = {
        ...
        isInFunction: beforeContext.some(
          (line) => /function\s+\w+\s*\(/.test(line) || /const\s+\w+\s*=\s*\(/.test(line) || /\w+\s*:\s*\(/.test(line),
        ),
  ...
  const triggerContextAwareSuggestion = useCallback(
    (editor: any, position: any) => {
  ...
  const acceptCurrentSuggestion = useCallback(() => {
    ...
    try {
      ...
      // Reset accepted flag after a short delay to prevent immediate re-triggering/double-handling
      setTimeout(() => {
        suggestionAcceptedRef.current = false
        console.log("suggestionAcceptedRef reset to false")
      }, 200) // Short delay to allow Tab handler to react
...
useEffect(() => {
  ...
  // Guard inside the effect (not in deps)
  if (isAcceptingSuggestionRef.current || suggestionAcceptedRef.current) {
  ...
  const handleEditorDidMount = (editor: any, monaco: Monaco) => {
    ...
    editor.addCommand(monaco.KeyCode.Tab, () => {
      ...
      // If the suggestion was just accepted, prevent default tab behavior for a brief moment
      if (suggestionAcceptedRef.current) {
        console.log("Suggestion was just accepted, preventing default tab behavior")
        return
      }
    ...
    editor.onDidChangeCursorPosition((e: any) => {
      ...
      if (
        ...
        !currentSuggestionRef.current &&
        !suggestionLoading &&
        (newPosition.lineNumber !== lastPosition.line || Math.abs(newPosition.column - lastPosition.column) > 2)
      ) {
