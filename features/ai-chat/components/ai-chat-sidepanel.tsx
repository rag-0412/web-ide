...
const MessageTypeIndicator: React.FC<{ type?: string; model?: string; tokens?: number }> = ({
  type,
  model,
  tokens,
}) => {
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h4 className="text-sm font-medium text-zinc-200">{suggestion.title}</h4>
              {suggestion.category && (
            ...
            {suggestion.fileName && (
              <div className="text-xs text-zinc-500">
                ...
                {suggestion.insertPosition && (
                  <span className="ml-2">
                    Line {suggestion.insertPosition.line}:{suggestion.insertPosition.column}
                  </span>
                )}
              </div>
            )}
          ...
          <div className="flex items-center gap-1">
            ...
            {onRun && ["javascript", "python", "bash"].includes(suggestion.language) && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => onRun(suggestion.code, suggestion.language)}
            ...
            <Tooltip>
              ...
              <TooltipContent>{activeFileName ? `Insert into ${activeFileName}` : "No active file"}</TooltipContent>
            </Tooltip>
          </div>
        </div>
      </div>
...
export const AIChatSidePanel: React.FC<AIChatSidePanelProps> = ({
    ...
    if (pastedText.length > 50 && pastedText.includes("\n")) {
      ...
      if (hasImports || hasFunctions || hasCodeStructure) {
        ...
        if (hasImports && pastedText.includes("React")) {
          ...
          suggestedName = pastedText.includes("interface") ? "script.ts" : "script.js"
        } else if (pastedText.includes("SELECT") || pastedText.includes("CREATE")) {
          suggestedName = "query.sql"
        } else if (pastedText.includes("<!DOCTYPE") || pastedText.includes("<html")) {
  ...
  const getChatModePrompt = (mode: string, content: string, context: any) => {
    const baseContext = {
      ...
      cursorPosition,
      attachments: attachments.map((f) => ({ name: f.name, language: f.language, size: f.size, type: f.type })),
    }
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-zinc-400">
                    {activeFileName ? `Working on ${activeFileName}` : "No active file"} • {messages.length} messages
                  </p>
                </div>
              </div>
              ...
              {filteredMessages.length === 0 && !isLoading && (
                <div className="text-center text-zinc-500 py-16">
                  ...
                  <p className="text-zinc-400 max-w-md mx-auto leading-relaxed mb-6">
                    Advanced AI coding assistant with file attachments, code execution, smart suggestions, and
                    ...
                    <div className="ml-14 space-y-2">
                      ...
                      {msg.suggestions.map((suggestion) => (
                        <CodeSuggestionCard
                          ...
                          onInsert={() =>
                            handleInsertCode(suggestion.code, suggestion.fileName, suggestion.insertPosition)
                          }
